...
export const BentoGridItem = ({
  ...
  const rightLists = ["VueJS", "Nextjs", "AWS"];
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText('abusay9901@gmail.com');
    setCopied(true);
  };
    ...
    >
      ...
      <div className={`${id === 6 ? "flex justify-center" : ""} h-full`}>
        <div className="w-full h-full absolute">
          {img && (
            <Image
              ...
              layout="fill" // Added layout property
              objectFit="cover" // Added objectFit property
            />
          )}
        </div>

        {/* Spare image for ID 5 */}
        <div
          className={cn(
        ...
        {id === 6 && (
          <BackgroundGradientAnimation>
            <div className="absolute z-50 inset-0 flex items-center justify-center text-white font-bold px-4 pointer-events-none text-3xl text-center md:text-4xl lg:text-7xl">
              {/* Add any text or element you want */}
            </div>
          </BackgroundGradientAnimation>
        )}
        ...
        >
          ...
          {/* Copy button and Lottie animation for ID 6 */}
          {id === 6 && (
